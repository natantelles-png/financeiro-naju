  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!user.email || !ALLOWED_EMAILS.includes(user.email.toLowerCase())) {
        await signOut(auth);
        setError('Acesso negado. Este aplicativo é exclusivo para Júlia & Natan.');
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl text-center space-y-6">
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-slate-100 hover:bg-white text-slate-900 rounded-xl px-4 py-3 font-semibold transition-colors flex items-center justify-center gap-3 disabled:opacity-50"
        >
